...
#             prev_obj = gg
        ...
        return x0 + torch.tensor(gg*xg, dtype=torch.float)
...
#             u = u.reshape([1,1,N,N])
            ...
            if (target is None and
                self.model.predict_label(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float)) != y0):
                sign = -1
            queries += 1
...
#
    ...
    def sign_grad_svm(self, x0, y0, theta, initial_lbd, h=0.001, K=100, lr=5.0, target=None):
        ...
        for iii in range(K):
            ...
            if (target is None and
                self.model.predict_label(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float)) != y0):
                sign = -1
                #if self.model.predict_label(x0+torch.tensor((initial_lbd*1.00001)*new_theta, dtype=torch.float)) == y0:
                #    print "Yes"
                #else:
                #    print "No"
    ...
    def eval_grad(self, model, x0, y0, theta, initial_lbd, tol=1e-5,  h=0.001, sign=False):
        ...
        while not it.finished:
            ...
            unit_x = x / LA.norm(x)
            if sign:
                if model.predict_label(x0+torch.tensor(initial_lbd*unit_x, dtype=torch.float)) == y0:
                    g = 1
                else:
                    g = -1
                q1 = 1
...
#             prev_obj = gg
        ...
        adv_target = model.predict_label(x0 + torch.tensor(gg*xg, dtype=torch.float))
        if (adv_target == target):
            timeend = time.time()
            print("\nAdversarial Example Found Successfully: distortion %.4f target"
                  " %d queries %d LS queries %d \nTime: %.4f seconds" % (gg, target, query_count, ls_total, timeend-timestart))

            return x0 + torch.tensor(gg*xg, dtype=torch.float), gg
        else:
    ...
    def fine_grained_binary_search_local_targeted(self, model, x0, y0, t, theta, initial_lbd=1.0, tol=1e-5):
        ...
        if model.predict_label(x0 + torch.tensor(lbd*theta, dtype=torch.float)) != t:
            lbd_lo = lbd
            lbd_hi = lbd*1.01
            nquery += 1
            while model.predict_label(x0 + torch.tensor(lbd_hi*theta, dtype=torch.float)) != t:
                lbd_hi = lbd_hi*1.01
                nquery += 1
                if lbd_hi > 100: 
                    return float('inf'), nquery
            ...
            lbd_lo = lbd*0.99
            nquery += 1
            while model.predict_label(x0 + torch.tensor(lbd_lo*theta, dtype=torch.float)) == t:
                lbd_lo = lbd_lo*0.99
                nquery += 1
        ...
        while (lbd_hi - lbd_lo) > tol:
            lbd_mid = (lbd_lo + lbd_hi)/2.0
            nquery += 1
            if model.predict_label(x0 + torch.tensor(lbd_mid*theta, dtype=torch.float)) == t:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
...
#         loss = np.sum(np.square(temp_theta))
    ...
    def fine_grained_binary_search_targeted(self, model, x0, y0, t, theta, initial_lbd, current_best):
        nquery = 0
        if initial_lbd > current_best: 
            if model.predict_label(x0 + torch.tensor(current_best*theta, dtype=torch.float)) != t:
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
